package com.json.basewebview.Utils.Permission;
        ...
        if (!ActivityCompat.shouldShowRequestPermissionRationale(activity, permission)) {
            ...
            switch (permission) {
                case Manifest.permission.CAMERA:
                    showToast(activity.getApplication(),"相机权限已被禁止，请在应用管理中打开权限");
                    break;
                case Manifest.permission.WRITE_EXTERNAL_STORAGE:
                    showToast(activity.getApplication(),"文件权限已被禁止，请在应用管理中打开权限");
                    break;
                case Manifest.permission.RECORD_AUDIO:
                    showToast(activity.getApplication(),"录制音频权限已被禁止，请在应用管理中打开权限");
                    break;
                case Manifest.permission.ACCESS_FINE_LOCATION:
                    showToast(activity.getApplication(),"位置权限已被禁止，请在应用管理中打开权限");
                    break;
            }
        } else {
            //用户没有勾选了不再询问,拒绝了权限申请
            switch (permission) {
                case Manifest.permission.CAMERA:
                    showToast(activity.getApplication(),"没有相机权限");
                    break;
                case Manifest.permission.WRITE_EXTERNAL_STORAGE:
                    showToast(activity.getApplication(),"没有文件读取权限");
                    break;
                case Manifest.permission.RECORD_AUDIO:
                    showToast(activity.getApplication(),"没有录制音频权限");
                    break;
                case Manifest.permission.ACCESS_FINE_LOCATION:
                   showToast(activity.getApplication(),"没有位置权限");
                    break;
            }
        }
    }
    private static void showToast(Context mCxt,String content){
        Toast.makeText(mCxt, content, Toast.LENGTH_SHORT).show();
    }
